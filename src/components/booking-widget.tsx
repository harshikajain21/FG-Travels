...
import { MapPin, Lock, Calendar, Users, Utensils, Wallet, ArrowRight, Loader2 } from "lucide-react";
...
export function BookingWidget() {
  ...
  useEffect(() => {
    if (!destination.trim() || !vehicleId) { setEstimate(null); return; }
    const t = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await getEstimateFn({ data: { destination, vehicleId, durationDays: days, driverFood: food } });
        if (res.ok) setEstimate(res); else setEstimate(null);
      ...
      <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
        ...
        <div className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              <Label htmlFor="dest" className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 mb-1.5">
              ...
              <datalist id="dest-list">
                {SUGGESTED_DESTINATIONS.map(d => <option key={d} value={d} />)}
              </datalist>
            </div>
          </div>
              ...
              {STATIC_CARS.map(c => (
                <button
                  ...
                  className={cn(
                    ...
                    vehicleId === c.id ? "border-accent bg-accent/5 shadow-sm" : "border-border hover:border-accent/40"
                  )}
                >
                  <img src={getCarImage(c.image_path)} alt={c.name} className="w-16 h-12 object-contain rounded" loading="lazy" />
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-sm font-bold text-accent mt-0.5">{fmt(c.rate_per_km)}/km</div>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <Label htmlFor="days" className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 mb-1.5">
                <Calendar className="size-3" /> Trip duration (days)
              </Label>
              <Input id="days" type="number" min={1} max={30} value={days}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDays(Math.max(1, Math.min(30, parseInt(e.target.value) || 1)))}
                className="h-11" />
            </div>
            <div>
              <Label className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 mb-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setFood("provide_locally")}
                  className={cn("h-11 px-2 rounded-md border-2 text-xs font-medium transition-all",
                    food === "provide_locally" ? "border-accent bg-accent/5" : "border-border hover:border-accent/40")}>
                  Provide locally
                </button>
                <button type="button" onClick={() => setFood("pay_allowance")}
                  className={cn("h-11 px-2 rounded-md border-2 text-xs font-medium transition-all",
                    food === "pay_allowance" ? "border-accent bg-accent/5" : "border-border hover:border-accent/40")}>
                  ₹250/day allowance
                </button>
              </div>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 pt-2 border-t border-border">
            <div>
              <Label htmlFor="name" className="text-xs font-medium text-muted-foreground mb-1.5 block">Your name</Label>
              <Input id="name" value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} className="h-11" placeholder="Full name" />
            </div>
            <div>
              <Label htmlFor="phone" className="text-xs font-medium text-muted-foreground mb-1.5 block">WhatsApp number</Label>
              <Input id="phone" value={phone} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPhone(e.target.value)} className="h-11" placeholder="+91…" />
            </div>
          </div>
          ...
          >
            {submitting ? <Loader2 className="size-4 animate-spin" /> : <>Submit & Book on WhatsApp <ArrowRight className="size-4 ml-1" /></>}
          </Button>
        </div>
        ...
        <div>
          <div className="rounded-xl border-2 border-dashed border-accent/40 bg-secondary/40 p-5 sticky top-20">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="text-xs uppercase tracking-widest text-muted-foreground">Live estimate</div>
                <div className="text-sm font-semibold">Transparent breakdown</div>
              </div>
            ...
            {!estimate ? (
              ...
              <div className="space-y-3 text-sm">
                <Row label={`${selectedCar?.name ?? "Vehicle"}`} sub={`${estimate.roundTripKm} km × ${fmt(estimate.ratePerKm)}/km`} value={fmt(estimate.baseFare)} />
                <Row label="Driver food" sub={food === "pay_allowance" ? `${days} day × ₹250` : "Provided by customer"} value={fmt(estimate.foodAllowance)} />
                <div className="pt-3 border-t border-border flex items-end justify-between">
                  <div>
                    <div className="text-xs text-muted-foreground">Estimated total</div>
                    <div className="text-3xl font-bold text-foreground">{fmt(estimate.total)}</div>
                  </div>
                  <div className="text-right text-[10px] text-muted-foreground max-w-[140px]">Tolls & diesel fluctuations payable at actuals</div>
                </div>
              </div>
            )}
            ...
            <Button
              ...
              style={{ background: "var(--gradient-accent)", color: "var(--accent-foreground)" }}
            >
              {submitting ? <Loader2 className="size-4 animate-spin" /> : <>Book on WhatsApp <ArrowRight className="size-4 ml-1" /></>}
            </Button>
